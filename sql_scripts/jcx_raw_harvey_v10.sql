...
INNER JOIN Payment P
    ...
    AND P.InstallmentID = Inst.InstallmentID
    AND P.PaymentMode IN ('A','D','K','B')
    --AND P.PaymentType IN ('~', 'X') --reset payments are in
    AND P.PaymentStatus = 'D'
WHERE Inst.DueDate < GETDATE()
GROUP BY A.Application_ID, A.PortFolioID, A.LoanID,
...
SELECT
    ...
    /* *************** isLoanDefault *************** */
    CASE
        -- 825: terminal only if NOT followed by recovery (111/779)
        WHEN A.installStatus = 825
             AND LEAD(A.installStatus) OVER (PARTITION BY A.LoanID
                                             ORDER BY A.InstallmentNumber)
                 NOT IN (111, 779)                                          THEN 1
        WHEN A.installStatus = 825
             AND LEAD(A.installStatus) OVER (PARTITION BY A.LoanID
                                             ORDER BY A.InstallmentNumber) IS NULL THEN 1
        WHEN A.installStatus = 825                                          THEN 0

        -- Deny new: pending install but loan is returned
        WHEN A.installStatus = 684
             AND A.LoanStatus IN ('R')
             AND A.InstallmentNumber = 1                                    THEN 1
         -- All voided: every installment is 115, loan never started
        ...
        -- Bad loan status + bad installment status
        WHEN A.LoanStatus NOT IN ('D', 'P')
             AND A.installStatus NOT IN (111, 115, 779, 684)               THEN 1

        -- Abandoned loan: paid install followed by pending, loan returned
        WHEN A.installStatus IN (111, 779)
             AND LEAD(A.installStatus) OVER (PARTITION BY A.LoanID
